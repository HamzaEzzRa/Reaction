...
const MainColorTypography = withStyles({
  root: {
    color: COLORS.MAIN_COLOR,
  },
})(Typography);
...
export default function Player({ onFullscreen, title, src, children, ...otherProps }) {
                ...
                <Grid container direction="column-reverse">
                  {[0.5, 1, 1.5, 2].map((rate, key) => (
                    <RateButton onClick={() => handlePlaybackRate(rate)} key={key} variant="text">
                      <MainColorTypography fontWeight={rate === state.playbackRate ? 'bold' : 'normal'}>
                        {rate}
                      </MainColorTypography>
                    </RateButton>
                  ))}
